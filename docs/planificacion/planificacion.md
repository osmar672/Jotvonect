...
## Objetivo
Construir una interfaz web de gestión de empleabilidad con autenticación y seis módulos CRUD.
...
## Requisitos
Funcionales: login, navegación, CRUD, confirmación de borrado, feedback y estado local.
No funcionales: accesibilidad básica, responsive, ES Modules, separación por responsabilidades, manejo de errores.

## Flujo
Login → token `jobconnect.token` → `index.html` → `requireAuth` → shell → módulo seleccionado → API → estado local → feedback.
...
Integrante 2: UI, servicios, vacancies, companies, applications e integración.
