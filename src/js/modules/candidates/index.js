...
export const moduleMeta = { id: "candidates", label: "Candidatos" };
export async function mount(root, injected) {
  container=root; services=injected; container.className="module module--candidates";
  container.innerHTML=`<div class="module-head"><div><p class="eyebrow">JOB CONNECT</p><h1>Candidatos</h1><p>Gestiona registros con operaciones seguras y estado local.</p></div><button class="btn btn--primary" data-action="new">+ Nuevo</button></div>
  <div class="module-toolbar"><input data-search type="search" placeholder="Buscar…" aria-label="Buscar"><span data-count></span></div>
  <form data-form class="entity-form" hidden><h2 data-form-title>Nuevo registro</h2><div class="form-grid"><label>Nombre<input name="firstName"  required></label>
<label>Apellido<input name="lastName"  required></label>
<label>Correo<input name="email"  required></label>
<label>Teléfono<input name="phone"  required></label></div><div class="form-actions"><button type="button" class="btn" data-action="cancel">Cancelar</button><button class="btn btn--primary" type="submit">Guardar</button></div></form>
  ...
  try { services.feedback.loading("Cargando candidatos…"); const result=await services.api.get("/users"); items=Array.isArray(result)?result:(result?.users||[]); services.feedback.clear(); renderList(); }
...
function openForm(item=null) {
 ...
 [('firstName', 'Nombre'), ('lastName', 'Apellido'), ('email', 'Correo'), ('phone', 'Teléfono')].forEach(([name])=>{const input=form.elements[name];if(input)input.value=item?.[name]??""});
 form.scrollIntoView({behavior:"smooth",block:"nearest"}); form.elements[[('firstName', 'Nombre'), ('lastName', 'Apellido'), ('email', 'Correo'), ('phone', 'Teléfono')][0][0]]?.focus();
}
...
async function submitForm(e) {
 ...
 const data={}; [('firstName', 'Nombre'), ('lastName', 'Apellido'), ('email', 'Correo'), ('phone', 'Teléfono')].forEach(([name])=>data[name]=form.elements[name].value);
 if([('firstName', 'Nombre'), ('lastName', 'Apellido'), ('email', 'Correo'), ('phone', 'Teléfono')].some(([name])=>name==="userId")) data.userId=Number(data.userId);
 ...
 try { services.feedback.loading(id?"Actualizando…":"Creando…"); const result=await services.api[method]("/users"+(id?"/"+id:""),data); if(id)items=items.map(x=>x.id===id?{...x,...result,...data}:x);else items=[{id:result?.id||Date.now(),...data,...result},...items]; services.feedback.success(id?"Registro actualizado.":"Registro creado."); closeForm();renderList(); }
...
async function deleteItem(id){if(!await services.feedback.confirmDelete("¿Eliminar este registro?"))return;try{ services.feedback.loading("Eliminando…");await services.api.remove("/users/"+id);items=items.filter(x=>x.id!==id);services.feedback.success("Registro eliminado.");renderList()}catch(error){services.feedback.error(error.message)}}
