...
export const moduleMeta = { id: "vacancies", label: "Vacantes" };
export async function mount(root, injected) {
  container=root; services=injected; container.className="module module--vacancies";
  container.innerHTML=`<div class="module-head"><div><p class="eyebrow">JOB CONNECT</p><h1>Vacantes</h1><p>Gestiona registros con operaciones seguras y estado local.</p></div><button class="btn btn--primary" data-action="new">+ Nuevo</button></div>
...
<label>Referencia salarial<input name="price"  required></label></div><div class="form-actions"><button type="button" class="btn" data-action="cancel">Cancelar</button><button class="btn btn--primary" type="submit">Guardar</button></div></form>
  ...
  try { services.feedback.loading("Cargando vacantes…"); const result=await services.api.get("/products"); items=Array.isArray(result)?result:(result?.products||[]); services.feedback.clear(); renderList(); }
...
function openForm(item=null) {
 ...
 [('title', 'Título'), ('description', 'Descripción'), ('category', 'Categoría'), ('price', 'Referencia salarial')].forEach(([name])=>{const input=form.elements[name];if(input)input.value=item?.[name]??""});
 form.scrollIntoView({behavior:"smooth",block:"nearest"}); form.elements[[('title', 'Título'), ('description', 'Descripción'), ('category', 'Categoría'), ('price', 'Referencia salarial')][0][0]]?.focus();
}
...
async function submitForm(e) {
 ...
 const data={}; [('title', 'Título'), ('description', 'Descripción'), ('category', 'Categoría'), ('price', 'Referencia salarial')].forEach(([name])=>data[name]=form.elements[name].value);
 if([('title', 'Título'), ('description', 'Descripción'), ('category', 'Categoría'), ('price', 'Referencia salarial')].some(([name])=>name==="userId")) data.userId=Number(data.userId);
 ...
 try { services.feedback.loading(id?"Actualizando…":"Creando…"); const result=await services.api[method]("/products"+(id?"/"+id:""),data); if(id)items=items.map(x=>x.id===id?{...x,...result,...data}:x);else items=[{id:result?.id||Date.now(),...data,...result},...items]; services.feedback.success(id?"Registro actualizado.":"Registro creado."); closeForm();renderList(); }
...
async function deleteItem(id){if(!await services.feedback.confirmDelete("¿Eliminar este registro?"))return;try{ services.feedback.loading("Eliminando…");await services.api.remove("/products/"+id);items=items.filter(x=>x.id!==id);services.feedback.success("Registro eliminado.");renderList()}catch(error){services.feedback.error(error.message)}}
